        ...
        while current_age < scenario.time_horizon.life_expectancy:
            ...
            for income in incomes:
                if income.time_bounds.is_active(current_age):
                    if income.kind == IncomeKind.ACTIVE:
                        income_breakdown.active[income.name] = income.monthly_amount
                    else:
                        income_breakdown.passive[income.name] = income.monthly_amount
            ...
            if net_cash_flow > 0:
                ...
                for asset in sorted_assets:
                    ...
                    tax_rate = new_asset.get_tax_rate(current_age)
                    if tax_rate:
            ...
            total_assets = sum(a.initial_value for a in assets)
            if total_assets <= 0:
                status = (
