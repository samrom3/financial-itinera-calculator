...
def test_simulator_success_scenario():
    ...
    scenario = (
        ...
        .with_expense(ExpenseBuilder("Living Expenses", monthly_amount=3_500).build())
        .with_tax_rate(
            TaxRateBuilder(0.22)
            .with_time_bounds(TimeBounds(end=retirement_goal.retirement_age))
            .build()
...
def test_simulator_post_retirement_bankruptcy_scenario():
    ...
    """
    time_horizon = TimeHorizon(
        current_age=Age(65, Month.JANUARY), life_expectancy=Age(95, Month.JANUARY)
    )
    retirement_goal = RetirementGoal(retirement_age=Age(65, Month.JANUARY))

    scenario = (
        FinancialScenarioBuilder(
            name="Post-Retirement Bankruptcy", time_horizon=time_horizon
        )
        .with_asset(AssetBuilder("Retirement Fund").with_initial_value(100_000).build())
        .with_expense(
            ExpenseBuilder("High Retirement Expenses", monthly_amount=5000).build()
        )
        .with_retirement_goal(retirement_goal)
        .build()
    )
