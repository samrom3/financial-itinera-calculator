...
def test_asset_helper_methods():
    ...
    asset = (
        AssetBuilder("Test Asset")
        .with_withdrawal_penalty(penalty)
        .with_override_withdrawal_tax(tax_rate)
        .with_contribution_constraint(constraint)
        .build()
    )
    ...
    assert asset.get_override_tax_rate(Age(60, Month.JUNE)) == tax_rate
    assert asset.get_override_tax_rate(Age(50, Month.JUNE)) is None
